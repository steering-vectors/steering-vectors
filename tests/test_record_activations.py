...
def test_record_activations_matches_decoder_hidden_states(
    ...
    with record_activations(model) as record_handle:
    ...
    assert len(record_handle.recorded_activations) == len(hidden_states) - 1
    for recorded_activation, hidden_state in zip(
        list(record_handle.recorded_activations.values())[:-1], hidden_states[1:-1]
    ):
