...
def train_steering_vector(
    ...
    for raw_sample in tqdm(
        ...
        sample = _formalize_sample(raw_sample)
        pos_prompt = sample.positive_prompt
        neg_prompt = sample.negative_prompt
        pos_token_idx = sample.read_positive_token_index
        neg_token_idx = sample.read_negative_token_index

        pos_index = _get_token_index(pos_token_idx, read_token_index, pos_prompt)
        neg_index = _get_token_index(neg_token_idx, read_token_index, neg_prompt)
...
def _formalize_sample(
    ...
    """
    Take a sample as tuple or as SteeringVectorTrainingSample and return it as
    SteeringVectorTrainingSample.
    """
